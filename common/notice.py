...
def listbypage(request):
    ...
    if 'withoutcontent' in request.params:
        data = models.news.objects.values('id','pubdate', 'author', 'author__realname', 'title', 'content',
                                            'status').filter(status=1).order_by('id')
    else:data = models.news.objects.values('id','pubdate','author','author__realname','title',
                                             'status').filter(status=1).order_by('id')
    if keywords:
        query = Q()
...
def listbypage_allstate(request):
    ...
    data = models.news.objects.values('id','pubdate', 'author', 'author__realname', 'title', 'content','status').order_by('id')
    if keywords:
        query = Q()
        conditions = [Q(title__contains=i) for i in keywords.split(' ')]
        for i in conditions:
...
def getone(request):
    ...
    getone = models.news.objects.filter(id = id).values('id','pubdate', 'author',
...
def addone(request):
    ...
    one = models.news.objects.create(title=data['title'],content=data['content'],author_id=request.session['id'])
...
def modifyone(request):
    ...
    modifyone = models.news.objects.get(id = id)
    if 'title' in newdata:
...
def deleteone(request):
    id = request.params['id']
    delone = models.news.objects.get(id = id)
    delone.delete()
    return JsonResponse({'ret':0})
...
def banone(request):
    id = request.params['id']
    banone = models.news.objects.get(id=id)
    banone.status = 3
    banone.save()
    return JsonResponse({'ret': 0,'status':banone.status})
def publishone(request):
    id = request.params['id']
    publishone = models.news.objects.get(id=id)
    publishone.status = 1
    publishone.save()
